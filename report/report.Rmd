...
- Average traffic while `r bru` was closed: `r round(during[orig == bcn, mean(N.x + N.y)])` flights per day (including departures and arrivals, `r  sprintf('%+.1f', during[orig == bcn, mean(N.x + N.y)] / before[orig == bcn, mean(N.x + N.y)] * 100 - 100)`%)
- Average traffic after `r bru` reopens: `r after[orig == bcn, mean(N.x + N.y)]` flights per day (including departures and arrivals, `r  sprintf('%+.1f', after[orig == bcn, mean(N.x + N.y)] / before[orig == bcn, mean(N.x + N.y)] * 100 - 100)`%)
...
- Average traffic while `r bru` was closed: `r format(round(mean(eu[date >= event[1] & date <= event[2], N])))` flights per day (`r  sprintf('%+.1f', mean(eu[date >= event[1] & date <= event[2], N]) / mean(eu[date < event[1], N]) * 100 - 100)`%)
- Average traffic after `r bru` reopens: `r format(round(mean(eu[date > event[2], N])))` flights per day (`r  sprintf('%+.1f', mean(eu[date > event[2], N]) / mean(eu[date < event[1], N]) * 100 - 100)`%)
...
```{r}
datatable(fucked, options = list(order = list(list(6, 'asc')), dom = 'tp', columnDefs = list(list(className = 'dt-center', targets = '_all'))), colnames = c('Airline', 'ICAO Code', 'Flights Before', 'Flights During', 'Flights After', 'Delta During', 'Delta After', '% During', '% After')) %>%
